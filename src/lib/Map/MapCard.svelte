...
>
  {#if scrolled}
    ...
    >
      <div class="flex flex-row items-center justify-center gap-2">
        <div
          class="hidden lg:contents bg-gray-200 rounded-lg text-xs p-2 max-h-8 w-10 m-4"
        >
      ...
      <div class="justify-center items-center flex flex-row gap-2">
        ...
        {#if garden.socials}
          <a
            href={garden.socials}
            target="_blank"
            class="w-8 hover:bg-blue-700 text-xs bg-blue-500 text-white px-2 py-1 rounded-md font-semibold flex gap-1"
          >
            {@html socials}
          </a>
        {/if}
        ...
        {#if garden.contact}
          <a
            href={garden.contact}
            target="_blank"
            class="w-8 hover:bg-pink-700 text-xs bg-pink-500 text-white px-2 py-1 rounded-md font-semibold flex gap-1"
          >
            {@html contact}
          </a>
        {/if}

        {#if garden.archivaldocs}
          <a
            href={garden.archivaldocs}
            target="_blank"
            class="bg-gray-200 rounded-lg text-xs mr-2 p-2 max-h-8 w-8 flex hover:bg-gray-300"
          >
            {@html archives}
          </a>
        {/if}
        ...
        {#if garden.interview_vids}
          <a
            href={garden.interview_vids}
            target="_blank"
            class="bg-gray-200 rounded-lg text-xs mr-2 p-2 max-h-8 w-8 flex hover:bg-gray-300"
          >
            {@html videos}
  ...
  {:else}
    <div class="text-center" out:slide={{ y: -10 }}>
      <h1 class="justify-center text-lg md:text-2xl lg:text-2xl font-bold my-2">
        {garden.garden}
      </h1>

      <div
        class="justify-center flex flex-row md:flex-row sm:flex-row gap-2 items-center"
      >
        <div class="mr-2 pointer-events-auto">
          <CarouselButton direction="left" onclick={next} />
        </div>
        ...
        {#if garden.socials}
          <a
            href={garden.socials}
            target="_blank"
            class="w-8 max-h-6 lg:w-28 hover:bg-blue-700 text-xs bg-blue-500 text-white px-2 py-1 rounded-md font-semibold flex gap-1"
          >
        ...
        {#if garden.contact}
          <a
            href="mailto:{garden.contact}"
            target="_blank"
            class="w-8 max-h-6 lg:w-24 hover:bg-pink-700 text-xs bg-pink-500 text-white px-2 py-1 rounded-md font-semibold flex gap-1"
          >
      ...
      <div
        class="justify-center flex flex-row md:flex-row sm:flex-row my-4 gap-2"
      >
        <i
          class="bg-gray-200 rounded-lg text-xs mr-2 p-2 max-h-8 w-10 md:w-10 lg:w-10"
        >
          {currentMapState.currentIndex + 1}/{gardens.length}
        </i>
  ...
  {/if}
  <div
    class={`items-start flex flex-col-reverse sm:flex-row justify-center gap-4 px-4 ${scrolled ? "my-16" : "mt-2"}`}
  >
    <div class="lg:w-3/4 align-text-top">
      <a href={garden.photos} target="blank">
        <img
          class="float-right ml-4 mb-2 w-[250px] rounded-md"
          alt={garden.alt}
